...
const buildFoundLine = (item: FoundItem): string =>
  `✓ Found ${item.category}${item.label ? ` — ${item.label}` : ""}${
    item.source_url && item.source_url !== "web_search" ? ` on ${item.source_url}` : ""
  }`;
...
const ChatbotRightSideView = ({
  ...
  useEffect(() => {
    ...
    const welcomeMsg: Message = {
      id: "welcome",
      role: "assistant",
      content: isTrainingComplete ? defaultContent : (welcomeMessage || defaultContent),
      timestamp: new Date(),
      isTyping: true,
    };
            ...
            {[
              ...
              <div key={node.label} className="flex items-center gap-3">
                <div className="flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-medium text-gray-400">{node.label}</span>
                </div>
                {i < arr.length - 1 && (
