...
function emptyMessengerSettings(): MessengerSettings {
  return { page_id: "", access_token: "", page_name: "" };
}

function emptyMessengerSnapshot(): MessengerSnapshot {
  return { settings: emptyMessengerSettings(), connected: false };
}

export async function getMessengerSnapshot(): Promise<{
  ok: boolean;
  snapshot?: MessengerSnapshot;
  error?: string;
}> {
  const result = await apiRequest<MessengerSnapshot>(
    "/api/apps-integration/messenger/snapshot",
    { method: "GET" },
    "Failed to fetch Messenger status",
  );
  ...
  return { ok: true, snapshot: result.data || emptyMessengerSnapshot() };
}

export async function connectMessenger(params: {
  pageId: string;
  accessToken: string;
}): Promise<{
  ok: boolean;
  ...
  const result = await apiRequest<MessengerSnapshot>(
    "/api/apps-integration/messenger/connect",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
// `pageId` is optional — Meta doesn't hand back a picked Page for a plain
// Facebook Login for Business the way WhatsApp's Embedded Signup posts back
// phone_number_id/waba_id, so the backend auto-selects it when the login
// only grants access to one Page.
export async function connectMessengerEmbedded(params: {
  code: string;
  pageId?: string;
}): Promise<{
  ok: boolean;
  ...
  const result = await apiRequest<MessengerSnapshot>(
    "/api/apps-integration/messenger/connect-embedded",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    "Failed to connect Messenger via Facebook",
  );
...
export async function disconnectMessenger(): Promise<{
  ok: boolean;
  error?: string;
}> {
